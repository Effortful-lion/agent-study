...
type ToolCallingParadigm interface {
	...
	Parse(content string) ([]core.ToolCall, error) // 解析内容提取工具调用列表
}
...
func (p *FunctionCallingParadigm) Parse(content string) ([]core.ToolCall, error) {
	var calls []core.ToolCall
	var data map[string]any
	if err := json.Unmarshal([]byte(content), &data); err != nil {
		lg.Frame.Error("FunctionCalling: 工具调用格式解析失败", lg.Fields{"error": err})
		return nil, errutil.NewAgentError(errutil.ErrCategoryModel, "工具调用格式解析失败", err, false)
	}
	if toolCalls, ok := data["tool_calls"].([]any); ok {
		for _, tc := range toolCalls {
			if call, ok := tc.(map[string]any); ok {
				...
				if name != "" {
					calls = append(calls, core.ToolCall{Name: name, Args: argsRaw})
				}
			}
		}
	}
	if function, ok := data["function"].(map[string]any); ok {
		...
		if name != "" {
			calls = append(calls, core.ToolCall{Name: name, Args: args})
		}
	}
	return calls, nil
}
...
func (p *ReActParadigm) Parse(content string) ([]core.ToolCall, error) {
	var calls []core.ToolCall
