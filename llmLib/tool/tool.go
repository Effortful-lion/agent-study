...
func (r *Registry) ToolDefs() []core.ToolDef {
...
func (r *Registry) Call(ctx context.Context, name string, args map[string]any) (any, error) {
	...
	if !ok {
		...
		return nil, errutil.NewAgentError(errutil.ErrCategoryToolNotFound, fmt.Sprintf("工具 %s 不存在", name), nil, false)
	}
	...
	if err := json.Unmarshal([]byte(argsJSON), &args); err != nil {
		...
		if json.Unmarshal([]byte(argsJSON), &argsStr) == nil {
			if json.Unmarshal([]byte(argsStr), &args) != nil {
				...
				return nil, errutil.NewAgentError(errutil.ErrCategoryTool, "参数解析失败: "+err.Error(), err, false)
			}
		} else {
			lg.Frame.Error("BuildArgs: 参数解析失败", lg.Fields{"args": argsJSON, "error": err})
			return nil, errutil.NewAgentError(errutil.ErrCategoryTool, "参数解析失败: "+err.Error(), err, false)
