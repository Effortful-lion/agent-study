...
// opts: 可选配置项，如 WithSystemPrompt、WithAgentBudgetConfig、WithStore
			...
			if len(toolCalls) > 0 {
				...
				for _, tc := range toolCalls {
					...
					state.Messages = append(state.Messages,
						...
						Message{Role: ToolRole, Content: result, ToolCallID: tc.ID},
					)
				}
			} else {
