...
func TestAgentBudget_ShouldStop(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:   "no limit",
			budget: AgentBudgetConfig{},
			state:  &State{Step: 100, Usage: core.Usage{InputTokens: 1000000}},
			want:   false,
		},
		{
		...
		},
		{
			...
			state:  &State{Usage: core.Usage{InputTokens: 60, OutputTokens: 50}},
			want:   true,
		},
		{
			name:   "not exceed max tokens",
			budget: AgentBudgetConfig{MaxTotalTokens: 100},
			state:  &State{Usage: core.Usage{InputTokens: 40, OutputTokens: 50}},
			want:   false,
		},
		{
...
func TestClassifyError(t *testing.T) {
	tests := []struct {
		...
		err        error
		statusCode int
		category   errutil.ErrorCategory
		retryable  bool
	}{
		{name: "nil error", err: nil, statusCode: 200, category: "", retryable: true},
		{name: "timeout", err: context.DeadlineExceeded, statusCode: 0, category: errutil.ErrCategoryTimeout, retryable: true},
		{name: "canceled", err: context.Canceled, statusCode: 0, category: errutil.ErrCategoryCanceled, retryable: false},
		{name: "auth 401", err: errors.New("status=401 unauthorized"), statusCode: 401, category: errutil.ErrCategoryAuth, retryable: false},
		{name: "auth 403", err: errors.New("status=403 forbidden"), statusCode: 403, category: errutil.ErrCategoryAuth, retryable: false},
		{name: "not found", err: errors.New("status=404 not found"), statusCode: 404, category: errutil.ErrCategoryNotFound, retryable: false},
		{name: "rate limited", err: errors.New("status=429 too many requests"), statusCode: 429, category: errutil.ErrCategoryRateLimited, retryable: true},
		{name: "5xx", err: errors.New("status=500 internal server error"), statusCode: 500, category: errutil.ErrCategoryProviderError, retryable: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotCategory, gotRetryable := errutil.ClassifyError(tt.err, tt.statusCode)
...
func (m *mockProvider) Chat(ctx context.Context, cfg core.LLMConfig, messages []core.Message) (*core.ChatResponse, error) {
...
func (m *mockProvider) ChatStream(ctx context.Context, cfg core.LLMConfig, messages []core.Message) (<-chan core.StreamChunk, error) {
	return nil, nil
}

func (m *mockProvider) ChatWithTools(ctx context.Context, cfg core.LLMConfig, messages []core.Message, tools []core.ToolDef) (*core.ChatResponse, error) {
	return m.Chat(ctx, cfg, messages)
}

func (m *mockProvider) ChatStreamWithTools(ctx context.Context, cfg core.LLMConfig, messages []core.Message, tools []core.ToolDef) (<-chan core.StreamChunk, error) {
	return nil, nil
}
...
func TestAgent_Run_WithToolCall(t *testing.T) {
	...
	mock := &mockProvider{
		responses: []*core.ChatResponse{
			{ToolCalls: []core.ToolCall{{ID: "call_1", Name: "get_time", Args: json.RawMessage("{}")}}},
...
func (m *mockReActProvider) Chat(ctx context.Context, cfg core.LLMConfig, messages []core.Message) (*core.ChatResponse, error) {
	if m.idx >= len(m.responses) {
		return &core.ChatResponse{Content: "done"}, nil
	}
	resp := m.responses[m.idx]
...
func (m *mockReActProvider) ChatStream(ctx context.Context, cfg core.LLMConfig, messages []core.Message) (<-chan core.StreamChunk, error) {
	return nil, nil
}
...
func TestAgent_Run_MultipleTools(t *testing.T) {
	...
	mock := &mockProvider{
		responses: []*core.ChatResponse{
			{ToolCalls: []core.ToolCall{{ID: "call_1", Name: "calculator", Args: json.RawMessage(`{"expression":"3+5*2"}`)}}},
			{ToolCalls: []core.ToolCall{{ID: "call_2", Name: "get_current_time", Args: json.RawMessage(`{}`)}}},
	...
	if finalAnswer != "3+5*2 的结果是 13，现在时间是 2024-01-01 12:00:00" {
		t.Errorf("final answer = %v, want %v", finalAnswer, "3+5*2 的结果是 13，现在时间是 2024-01-01 12:00:00")
	}
}
